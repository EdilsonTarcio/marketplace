...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>Atualização de Segmento Categoria</h1>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Atualizar Segmento de Categoria</h4>
                            <div class="card-header-action">
                                <a href="#" class="btn btn-primary"> Ajuda?</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('categoria-segmento.update', $categoriaSegmento->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                    ...
                                    <select name="id_categoria" class="form-control categoria-form">
                                        @foreach ($categorias as $categoria )
                                            <option value="{{ $categoria->id }}"
                                                {{ $categoria->id == $categoriaSegmento->id_categoria ? 'selected' : null}}>
                                    ...
                                    <select name="id_subcategoria" class="form-control subcategoria-form">
                                        @foreach ($subCategorias as $subCategoria )
                                            <option value="{{ $subCategoria->id }}"
                                                {{ $subCategoria->id == $categoriaSegmento->id_sub_categoria ? 'selected' : null}}>
                                                {{ $subCategoria->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="row">
                                    <div class="form-group col-6">
                                        ...
                                        <input type="text" name="name" class="form-control"
                                            value="{{ old('name', $categoriaSegmento->name) }}">
                                    </div>
                                    <div class="form-group col-6">
                                        <label for="">Status</label>
                                        <select name="status" class="form-control">
                                            <option value="1" {{ $categoriaSegmento->status == 1 ? 'selected' : null}}>Ativo</option>
                                            <option value="0" {{ $categoriaSegmento->status == 0 ? 'selected' : null}}>Inativo</option>
                                        </select>
                                    </div>
                                </div>
